<template>
    <div class="team-row">
        <div v-if="!loading" class="text"><strong>{{ team.dmg.toLocaleString() }}</strong> with {{ team.crit_rate }}%
            crit rate </div>
        ...
        <div v-if="!loading" class="text">{{ team.attacker_crys1 }} {{ team.attacker_crys2 }} {{ team.attacker_crys3 }}
        </div>
    </div>
</template>
